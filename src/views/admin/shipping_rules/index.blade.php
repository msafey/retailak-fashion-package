...
@component('layouts.admin.breadcrumb')
                ...
                <!-- Add Company Button -->
                <a href="{{url('/admin/shipping-rules/create')}}" class="btn btn-rounded btn-primary"><i
                            class="zmdi zmdi-plus-circle-o"></i> Add New Shipping Rule</a>
...
<script>
 ...
 <?php $editurl = url('/admin/shipping-rules/');?>
 <?php $changestatus = url('/admin/shipping-rules/status'); ?>

 var table = $('#items_datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{!! route('shippingrulesList') !!}',
        columns: [

        {
            data: 'shipping_rule_label',name:'shipping_rule_label'
        },
        ...
        },
        {
...

          {
               data: 'id', render: function (data, data2, type, row) {
                     ...
                     var deactivate = "<a href='{{$changestatus}}" + '/' + data + "' class='btn btn-danger'>Disable &nbsp;";
                     var activate = "<a href='{{$changestatus}}" + '/' + data + "' class='btn btn-danger'>Enable &nbsp;";
                    if (type.disabled == 0) {
