...
@component('layouts.admin.breadcrumb')
        ...
        {!! Form::open(['url' => ['/admin/suppliers', $supplier->id],'method'=>'PATCH', 'id'=>'form','files' => true, 'data-parsley-validate'=>'']) !!}
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        <div class="col-sm-6" style="margin-top: 0px">
                                            <div class='input-group date' style="display: inline;" id='datetimepicker1'>
                                                <input type='text' value="{{$supplier->name}}" required name="name" class="form-control">
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class='input-group date' id='datetimepicker1' style="display: inline;">
                                                <select required name="supplier_type_id" id="sup_type" class="form-control" >
                                                    ...
                                                    <?php foreach ($supplier_types as $type) { ?>
                                                        <option value="{{$type->id}}" @if($supplier->supplier_type_id == $type->id)selected @endif>{{$type->name}}</option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                        </div>
