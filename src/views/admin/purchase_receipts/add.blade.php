...
@component('layouts.admin.breadcrumb')
                ...
                {!! Form::open(['url' => '/admin/purchase-receipts', 'class'=>'form-hirozontal ','id'=>'demo-form','files' => true, 'data-parsley-validate'=>'']) !!}
                ...
                <div class="card card-block">
                  ...
                  <div class="row">
                        ...
                        <div class="col-lg-5">
                          ...
                          <div class="col-sm-12" >
                            <div class='input-group date' id='datetimepicker1'>
                                <input type='text' name="required_by_date" disabled="disabled" value="{{date ('m/d/Y h:i:m',strtotime($order->required_by_date))}}" class="form-control"/>
                      ...
                      <div class="row">
                            ...
                            <table id="myTable"  class=" table order-list" style="width: 850px;">
                            <thead>
                                <tr>
                                    <td>Items</td>
                                    <td>Quantity</td>
                                    <td>Remained Quantity</td>
                            ...
                            @if(count($item_details)>0)
                                @foreach($item_details as $order_item)
                                    <tr id="field{{$i}}">
                                        <td class="col-sm-3">
                                          <div class='input-group date' id='' style="display: inline;">
                                            <input type="text"  hidden="hidden" name="item_id[]" value="{{$order_item->item_id}}">
                                            <input type="text" hidden name="rate" id="rate" value="{{$order_item->rate}}">
                                        ...
                                        <td class="col-sm-2">
                                            <input type="number" disabled="disabled"     name="" value="{{$order_item->remained_qty}}" id="remained_qty"  class="form-control    remained_qty"/>
                                        </td>
                                        <td class="col-sm-2">
                                            <input type="number"  step="1" @if($order_item->remained_qty != 0) max="{{$order_item->remained_qty}}" min="1" @endif  value="{{$order_item->remained_qty}}" id="accepted_qty{{$order_item->item_id}}"  name="accepted_qty[]" class="form-control    accepted_qty"/>
                                        </td>
                                       <!--  <td class="col-sm-1">
                            ...
                            @if(count($parent_array)>0)
                                @foreach($parent_array as $key => $parent_item)
                                <tr>
                                  ...
                                  @foreach($parent_item as $item)
                                    <tr id="field{{$i}}">
                                        <td class="col-sm-3">
                                          <div class='input-group date' id='' style="display: inline;">
                                            <input type="text"  hidden="hidden" name="item_id[]" value="{{$item->item_id}}">
                                            <input type="text" hidden name="rate" id="rate" value="{{$item->rate}}">
                                              <input type="text" disabled="disabled" class="form-control" name="" value="{{$item->product_name}}">
                                          </div>                                    
                                        </td>

                                        <td class="col-sm-2">
                                            <input type="number" disabled="disabled"  min="1"   name="" value="{{$item->qty}}" id="qty"  class="form-control qty"/>
                                        </td>
                                        <td class="col-sm-2">
                                            <input type="number" disabled="disabled"     name="" value="{{$item->remained_qty}}" id="remained_qty"  class="form-control    remained_qty"/>
                                        </td>
                                        <td class="col-sm-2">
                                            <input type="number"  step="1" @if($item->remained_qty != 0) max="{{$item->remained_qty}}" min="1" @endif  value="{{$item->remained_qty}}" id="accepted_qty{{$item->item_id}}"  name="accepted_qty[]" class="form-control    accepted_qty"/>
                                        </td>
                                       <!--  <td class="col-sm-1">
                      ...
                      <input type="hidden" value="{{$_GET['purchase_order_id']}}" name="purchase_order_id">
                      <div class="row" style="display: none;">
                        ...
                        <div class="col-lg-8" style="margin-top: 0px">
                                <div class="col-lg-4">
                                  ...
                                  <input type="text" disabled="disabled" class="form-control" value="{{$total_discount}} LE">
                                </div>
                                <div class="col-lg-4">
                     ...
                     <div class="row">
                       <div class="col-lg-6" style="display: none;left:250px;width: 515px ; height:150px;padding:10px;border: 2px solid gray; margin-top: 100px;">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $(document).on('input', '.accepted_qty', function(e){
        var attr_id = $(this).closest('tr').attr('id');   
        // console.log(attr_id);   
        var rate = $('#'+attr_id+' #rate').val();
        // console.log(rate);
        var accepted_qty_id = $(this).attr('id');
        var qty = $('#'+attr_id+' #'+accepted_qty_id).val();
...
function add(a, b) {
    ...
    function grandTotalAmount(){
      var total_accepted_qty=0;
        ...
        var ids = $('.accepted_qty').map(function() {
          return $(this).attr('id');
        });
        $.each(ids,function(index,value){
          total_accepted_qty += parseInt($('#'+value).val());
        });
