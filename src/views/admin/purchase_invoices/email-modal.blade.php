    ...
    $(document).on('click','.cancel',function(event){
      ...
      $(document).on('click','.save',function(event){
        ...
        if (email == "") {
            ...
            if($('.saved').length <= 0){
                $('#saved').append("<img src='{{url('public/admin/images/pleasewait.gif')}}' id='myElem' style='width: 50px;height: auto;'' alt=''>");
            }
        ...
        $.ajax({          
            ...
            type: "POST",
            data: {'email': email,'body':body,'company_name':company_name,'purchase_receipt_id':purchase_receipt_id            },
            success: function (data) {
              // $('#myElem').append('<p>loading....<p>');
              if(data == 'true'){
                $('#myElem').hide();
                $('#saved').append("<img src='{{url('public/admin/images/okok.gif')}}' id='myElem' style='width: 50px;height: auto;'' alt=''>");
                $("#saved").show().delay(5000).queue(function(n) {
                        
                        $('#myModal4').modal('hide');

                  });

                // $('#myElem').append('<p>Email Sent Successfully .. </p>')
