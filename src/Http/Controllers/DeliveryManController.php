...
class DeliveryManController extends utilitiesController
{
        ...
        public function store(AddDeliveryManRequest $request)
    {
        // dd($request->all());
                ...
                $user = Auth::guard('admin_user')->user();
                       ...
                       $delivery_man->adjustments()->attach($user->id, ['key' =>"DeliveryMan", 'action' =>"Added",'content_name'=>$delivery_man->name]);
    ...
    public function update(EditDeliveryManRequest $request, $id){
        ...
        $user = Auth::guard('admin_user')->user();
               ...
               $delivery_man->adjustments()->attach($user->id, ['key' =>"DeliveryMan", 'action' =>"Edited",'content_name'=>$delivery_man->name]);
